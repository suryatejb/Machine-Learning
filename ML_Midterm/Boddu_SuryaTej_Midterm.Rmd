---
title: "INFSCI 2595 Fall 2025 - MIDTERM"
subtitle: "Assigned October 9, 2025; Due: October 15, 2025"
author: "Surya Tej Boddu"
date: "Submission time: Octover 15, 2025 at 11:59PM EST"
...
What do you think?  
The prior specification of a=0.5 and b=0.5 corresponds to one total prior trial, calculated as the sum of the shape parameters (a + b). This is considered "uninformed" because a weight of only one trial is statistically minimal compared to the observed data, ensuring that the likelihood quickly dominates the posterior. This is the Jeffreys prior, a standard default choice that does not depend on how we choose to measure or express the parameter. This makes it a standard choice in Bayesian analysis when no specific domain knowledge is available to justify a more informative prior.
...
What do you think?  
Based on the use of a Binomial likelihood and a conjugate Beta prior, the posterior distribution for the unknown event probability, $\mu_j$, is a Beta distribution.In Bayesian analysis, the Beta distribution is the conjugate prior for the Binomial likelihood. This means that when we multiply the Binomial likelihood (representing the observed catches and targets) by a Beta prior, the resulting posterior distribution mathematically remains within the Beta family. This conjugacy allows us to update our beliefs about a player's catch rate simply by adding the observed data to the prior shape parameters.
...
prior_q05  <- qbeta(0.05, a_prior, b_prior)
prior_q95  <- qbeta(0.95, a_prior, b_prior)

tibble::tibble(
  Statistic = c("Mean", "0.05 Quantile", "0.95 Quantile", "90% Interval Width"),
  Value = c(prior_mean, prior_q05, prior_q95, prior_q95 - prior_q05)
)
```
...
post_df_focus_from_vague <- df_focus %>% 
  mutate(
    anew = a_prior + num_events,
    bnew = b_prior + (num_trials - num_events)
  )
```
...
summary_post_df_focus_from_vague %>% 
  ...
  geom_linerange(aes(ymin = post_q05, ymax = post_q95), color = "gray") +
  geom_point(aes(y = post_avg, color = "Posterior Mean"), size = 2) +
  geom_point(aes(y = mle, color = "MLE"), shape = 4, size = 3, stroke = 1.5) +
  labs(x = "Player ID", 
       y = "Catch Rate", 
       title = "Posterior Uncertainty vs. MLE (Uninformed Prior)",
       color = "Estimate Type") +
  theme_bw()
```

What do you think?  
In this unpooled analysis with an uninformed prior, there are no players with MLEs outside the posterior uncertainty interval. Because the prior is vague (a=0.5, b=0.5), it contributes only one pseudo-trial of weight, allowing the likelihood of the observed data to dominate the results. Consequently, the posterior mean values are extremely close (and often identical to several decimal places) to the individual MLEs. This demonstrates that without an informative prior, our Bayesian estimate effectively defaults to the frequentist point estimate, providing little additional value beyond quantifying the range of uncertainty.
...
summary_post_df_focus_from_vague %>% 
  ...
  # Use position_dodge to prevent overlapping points for players with the same trials
  geom_linerange(aes(ymin = post_q05, ymax = post_q95), 
                 position = position_dodge(0.2), color = "gray") +
  geom_point(aes(y = post_avg, color = "Posterior Mean"), 
             position = position_dodge(0.2), size = 2) +
  geom_point(aes(y = mle, color = "MLE"), 
             position = position_dodge(0.2), shape = 4, size = 3) +
  labs(x = "Number of Trials (Targets)", 
       y = "Catch Rate", 
       title = "Uncertainty vs. Sample Size") +
  theme_bw()
```

What do you think?  
Based on the visualization, the players with the lowest number of trials (the smallest num_trials) exhibit the highest posterior uncertainty. This is visually represented by the longest vertical lines in the plot, indicating a wider 90% uncertainty interval. In a Bayesian framework with an uninformed prior, a small sample size provides less information to the likelihood, resulting in a broader posterior distribution and less certainty about the true catch rate.
...
summary_post_df_all_from_vague %>% 
  mutate(mle = num_events / num_trials) %>% 
  ggplot(aes(x = mle, y = post_avg)) +
  geom_point(aes(color = num_trials), alpha = 0.5) +
  geom_abline(slope = 1, intercept = 0, color = "red", linetype = "dashed") +
  scale_color_viridis_c() +
  labs(x = "Maximum Likelihood Estimate (m/N)", 
       y = "Posterior Mean (Uninformed Prior)", 
       color = "Number of Trials",
       title = "Posterior Mean vs. MLE (Uninformed Prior)") +
  theme_bw()
```
...
To derive the log-posterior $\log(p(\mu \mid \ldots))$, we start with the provided proportionality:

$$
p\left(\mu \mid \left((m,N)_j\right)_{j=1}^{J}\right) \propto \prod_{j=1}^{J} \text{Binomial}(m_j \mid \mu, N_j) \times \text{Beta}(\mu \mid a, b)
$$

Expand the Likelihood and Prior:
$$
\prod_{j=1}^{J}\left[\binom{N_j}{m_j}\mu^{m_j}(1-\mu)^{N_j-m_j}\right] \times \left[\frac{1}{B(a,b)}\mu^{a-1}(1-\mu)^{b-1}\right]
$$
Take the Natural Log:
$$
\log(p(\mu \mid \ldots)) \propto \sum_{j=1}^{J}\left[\log\binom{N_j}{m_j} + m_j\log(\mu) + (N_j-m_j)\log(1-\mu)\right] + \log\left(\frac{1}{B(a,b)}\right) + (a-1)\log(\mu) + (b-1)\log(1-\mu)
$$
Drop Constants (terms not containing $\mu$):

$$
\log(p(\mu \mid \ldots)) \propto \sum_{j=1}^{J}\left[m_j\log(\mu) + (N_j-m_j)\log(1-\mu)\right] + (a-1)\log(\mu) + (b-1)\log(1-\mu)
$$
Distribute the Summation:
$$
\log(p(\mu \mid \ldots)) \propto \left(\sum_{j=1}^{J}m_j\right)\log(\mu) + \left(\sum_{j=1}^{J}(N_j-m_j)\right)\log(1-\mu) + (a-1)\log(\mu) + (b-1)\log(1-\mu)
$$
Final Grouped Form
$$
\log\left(p\left(\mu \mid \left((m,N)_j\right)_{j=1}^{J}\right)\right) \propto \left(\sum_{j=1}^{J}m_j + a - 1\right)\log(\mu) + \left(\sum_{j=1}^{J}(N_j - m_j) + b - 1\right)\log(1-\mu)
$$
...
Add as many equation blocks as you feel are necessary to show the steps to derive the answer.  
$$
\log\left(p\left(\mu \mid \left((m,N)_j\right)_{j=1}^{J}\right)\right) \propto (J\bar{m} + a - 1)\log(\mu) + (J\bar{N} - J\bar{m} + b - 1)\log(1-\mu)
$$
$$
\log\left(p\left(\mu \mid \left((m,N)_j\right)_{j=1}^{J}\right)\right) \propto (J\bar{m} + a - 1)\log(\mu) + (J(\bar{N} - \bar{m}) + b - 1)\log(1-\mu)
$$
...
What do you think?  
The unknown "pooled" estimate $\mu$ follows a Beta distribution. This is because the Beta distribution is the conjugate prior for the Binomial likelihood; when we pool all independent trials across the league ($J\bar{N}$) and all successful events ($J\bar{m}$), the mathematical structure of the posterior remains within the Beta family.

Add as many equation blocks as you feel are necessary to show the steps to derive the answer.  
$$
a_{new} = \sum_{j=1}^{J} m_j + a = J\bar{m} + a
$$

$$
b_{new} = \sum_{j=1}^{J} (N_j - m_j) + b = J(\bar{N} - \bar{m}) + b
$$
...
M_all <- sum(df_all$num_events)
N_all <- sum(df_all$num_trials)

anew_pooled <- a_prior + M_all
bnew_pooled <- b_prior + (N_all - M_all)
...
```{r, solution_02f}
# Calculate the MLE for the entire league (the pooled MLE)
mle_pooled <- sum(df_all$num_events) / sum(df_all$num_trials)

summary_post_df_focus_pooled %>% 
  ggplot(aes(x = as.factor(player_id))) +
  geom_linerange(aes(ymin = post_q05, ymax = post_q95), color = "gray") +
  geom_point(aes(y = post_avg, color = "Posterior Mean"), size = 2) +
  geom_point(aes(y = mle_pooled, color = "Pooled MLE"), shape = 4, size = 3) +
  labs(x = "Player ID", 
       y = "Catch Rate", 
       title = "Completely Pooled Posterior Summaries vs. Pooled MLE",
       color = "Estimate Type") +
  theme_bw()
```

What do you think?  
No players have MLEs outside the posterior uncertainty interval. Because the model pools data from the entire league, the aggregate sample size is massive, which results in an extremely narrow 90% uncertainty interval centered on the global average. So, the posterior mean for every player is identical to the pooled MLE, as the sheer volume of data in the likelihood completely overwhelms the influence of the prior. Furthermore, every player shares the identical posterior distribution and posterior mean, regardless of their individual number of targets or catches, since the pooled model assigns the same $\mu$ to all players.
...
What do you think?  
The completely pooled estimate assumes that every player has the same unknown catch rate $\mu$. This means regardless of how many targets a player has or how many catches they made, every player receives the identical posterior distribution. A player with 5 targets and 4 catches is treated the same as a player with 200 targets and 160 catches — both get the same posterior mean and uncertainty interval.This is clearly wrong for this application because players genuinely differ in their catching ability. Some receivers are elite pass-catchers while others struggle. Forcing all players to share a single $\mu$ completely ignores individual player talent and performance. In other words, the pooled model has no ability to differentiate between players as it cannot answer the question "who is the best receiver?" because everyone has the same posterior. This makes it useless for talent evaluation purposes.
...
df_all %>% 
  mutate(catch_rate = num_events / num_trials,
         trial_group = case_when(num_trials <= 24 ~ "1-24 Trials",
                                 num_trials > 24 ~ "More than 24 Trials")) %>% 
  ggplot(aes(x = catch_rate)) +
  geom_histogram(binwidth = 0.05, fill = "steelblue", color = "white") +
  facet_wrap(~trial_group, scales = "free_y") +
  labs(x = "Catch Rate", 
       y = "Count", 
       title = "Catch Rate Distribution: Low vs. High Trial Counts") +
  theme_bw()
```
What do you think?  
The observations for players with more than 24 targets exhibit a much more stable and bell-shaped distribution. Unlike the low-trial group, which displays significant spikes at 0.0 and 1.0 due to the high variability of small sample sizes (e.g., catching 0 out of 1 or 1 out of 1), the high-trial group clusters more reliably around the league average. The distribution for these experienced players appears to center around a catch rate of approximately 0.60–0.65, providing a concrete preview of where the informative prior's mean will eventually settle. This stability makes the high-trial group the ideal foundation for estimating the a and b shape parameters in the Empirical Bayes process.
...
my_beta_loglik <- function(unknowns, my_info)
{
  ...
  # Since we transformed log(a) -> a, the adjustment is log(a) + log(b)
...
Yes. While the starting points were different—one beginning at zeros c(0, 0) and the other at c(-1, -1)—the optimization process converged to the same region of the parameter space. This indicates that the log-likelihood surface for the Beta distribution parameters is likely unimodal and well-behaved, allowing the BFGS algorithm to reach the global maximum regardless of these specific initial values.
...
log_ab_res_01 <- optim(
  ...
  my_info = info_for_ab,
  method = "BFGS",
  control = list(fnscale = -1), # MAXIMIZE the log-likelihood
  gr = NULL
)

# Display results for first guess
log_ab_res_01$par
```
...
# Display results for second guess
log_ab_res_02$par
```

**Are the identified log-transformed estimates the same?**  
Yes.Initial guess 01 resulted in 2.759740 and 2.058504. Initial guess 02 resulted in 2.759736 and 2.058500.The difference between these two results is less than $1 \times 10^{-5}$, which is within the standard numerical tolerance for the optim() function. These values are identical, confirming that both runs identified the same maximum likelihood estimates for the log-transformed shape parameters a and b.
...
The number of a-priori trials is the sum of the shape parameters (a + b). Based on the optimization results ($a \approx 15.8, b \approx 7.8$), this represents approximately 23.6 total prior trials. This value represents the weight of the prior i.e., an NFL player with only a few targets will have their catch rate significantly pulled toward this prior mean, while a player with hundreds of targets will see their data dominate the result.
...
df_24 %>% 
  ...
  geom_line(data = prior_for_viz, 
            ...
            color = 'red', 
            size = 1.15) +
  labs(x = "Observed Catch Rate (m/N)", 
       y = "Density", 
       title = "Empirical Bayes Prior vs. Observed Catch Rates (Trials > 24)") +
  theme_bw()
```

How does the empirically derived prior distribution on the event probability compare to the observed histogram of the catch rates?
The empirically derived prior distribution (the red line) aligns very well with the observed histogram of catch rates for the experienced players. By estimating the shape parameters a and b directly from the filtered data (df_24), the resulting Beta distribution accurately captures the central tendency (peaking around 0.67) and the variance seen in the actual performance of the players. This demonstrates that the Empirical Bayes approach effectively learns a representative prior from the population. This provides a more realistic foundation for evaluating talent than the uninformed prior used in Problem 1.
...
Add as many code chunks as you feel are necessary.  
```{r, solution_03h}
a_prior_eb <- ab_emp_bayes[1]
b_prior_eb <- ab_emp_bayes[2]

prior_eb_mean <- a_prior_eb / (a_prior_eb + b_prior_eb)
prior_eb_q05  <- qbeta(0.05, a_prior_eb, b_prior_eb)
prior_eb_q95  <- qbeta(0.95, a_prior_eb, b_prior_eb)

tibble::tibble(
  Statistic = c("Mean", "0.05 Quantile", "0.95 Quantile", "90% Interval Width"),
  Value = c(prior_eb_mean, prior_eb_q05, prior_eb_q95, prior_eb_q95 - prior_eb_q05)
)
```
...
```{r, solution_03i}
uninformed_mean <- 0.5
uninformed_q05  <- qbeta(0.05, 0.5, 0.5)
uninformed_q95  <- qbeta(0.95, 0.5, 0.5)

informative_mean <- ab_emp_bayes[1] / (ab_emp_bayes[1] + ab_emp_bayes[2])
informative_q05  <- qbeta(0.05, ab_emp_bayes[1], ab_emp_bayes[2])
informative_q95  <- qbeta(0.95, ab_emp_bayes[1], ab_emp_bayes[2])

tibble::tibble(
  Prior_Type = c("Uninformed", "Empirical Bayes"),
  Mean = c(uninformed_mean, informative_mean),
  `0.05_Quantile` = c(uninformed_q05, informative_q05),
  `0.95_Quantile` = c(uninformed_q95, informative_q95),
  `90%_Interval_Width` = c(uninformed_q95 - uninformed_q05, informative_q95 - informative_q05)
)
```

What do you think?  
The transition from a vague uninformed prior to an informative Empirical Bayes prior represents a fundamental shift in our baseline assumptions for the NFL catch rate model. While the original uninformed prior assumed a mean $\mu = 0.50$, a coin flip that indicated a lack of prior domain knowledge, the Empirical Bayes prior has shifted that expectation to approximately 0.67, aligning it with the actual performance standards observed across professional NFL players.
  In addition, the uncertainty has been dramatically refined. The uninformed prior was extremely vague, with a 90% interval covering nearly the entire range from 0.006 to 0.994, whereas the informative prior is much more concentrated with an interval width of roughly 0.31. This increased precision is a result of the "prior weight" increasing from a single trial to approximately 23.6 pseudo-trials. Together, these differences demonstrate that the Empirical Bayes prior will shrink player estimates toward 0.67, especially for players with few targets, while still allowing players with large sample sizes to be driven primarily by their own data.
...
# Extract the informative shape parameters from the Empirical Bayes optimization
a_prior_eb <- ab_emp_bayes[1]
b_prior_eb <- ab_emp_bayes[2]

# Calculate the informative posterior shape parameters for each player
post_df_focus_empbayes <- df_focus %>% 
  mutate(
    anew = a_prior_eb + num_events,
    bnew = b_prior_eb + (num_trials - num_events)
  )
```
...
summary_post_df_focus_empbayes %>% 
  ...
  ggplot(aes(x = as.factor(player_id))) +
  geom_linerange(aes(ymin = post_q05, ymax = post_q95), color = "gray") +
  geom_point(aes(y = post_avg, color = "EB Posterior Mean"), size = 2) +
  geom_point(aes(y = mle, color = "MLE"), shape = 4, size = 3, stroke = 1.5) +
  labs(x = "Player ID", 
       y = "Catch Rate", 
       title = "Empirical Bayes Posterior vs. MLE (Focused Players)",
       color = "Estimate Type") +
  theme_bw()
```

How does this visualization compare to those you made using the vague unpooled estimate and the completely pooled estimate?
The visualization of the Empirical Bayes posterior summaries represents a middle ground between the unpooled and completely pooled extremes explored earlier. Unlike the uninformed model, where the posterior mean was identical to the MLE for every player, the Empirical Bayes approach results in some shrinkage. Player estimates are pulled away from noisy MLEs and toward the league-wide average of approximately 0.67. This effect is most pronounced for players with small sample sizes, where the informative prior provides approximately 23.6 trials of baseline weight to stabilize the estimate. Conversely, players with many targets have posterior means that remain close to their individual MLEs, since the volume of data in the likelihood eventually dominates the influence of the prior. Ultimately, this method allows players to remain distinct while borrowing strength from the broader population to provide more reliable talent evaluations than either extreme could offer on its own.
...
summary_post_df_focus_empbayes %>% 
  mutate(mle = num_events / num_trials) %>% 
  ggplot(aes(x = as.factor(num_trials), group = player_id)) +
  # Apply dodging to separate players with identical trial counts
  geom_linerange(aes(ymin = post_q05, ymax = post_q95), 
                 position = position_dodge(0.2), color = "gray") +
  geom_point(aes(y = post_avg, color = "EB Posterior Mean"), 
             position = position_dodge(0.2), size = 2) +
  geom_point(aes(y = mle, color = "MLE"), 
             position = position_dodge(0.2), shape = 4, size = 3) +
  labs(x = "Number of Trials (Targets)", 
       y = "Catch Rate", 
       title = "EB Uncertainty vs. Sample Size (Focused Players)") +
  theme_bw()
```
...
summary_post_df_all_empbayes %>% 
  ...
  scale_color_viridis_c() +
  labs(x = "Maximum Likelihood Estimate (m/N)", 
       y = "Posterior Mean (Empirical Bayes Prior)", 
       color = "Number of Trials",
       title = "Posterior Mean vs. MLE (Informative Prior)") +
  theme_bw()
```
...
summary_post_df_all_empbayes %>% 
  ...
  labs(x = "Number of Trials (Targets)", 
       y = "Posterior 90% Interval Range", 
       title = "Uncertainty Range vs. Sample Size (Informative Prior)") +
  theme_bw()
```
...
compare_post_summaries %>% 
  mutate(interval_range = post_q95 - post_q05) %>% 
  ggplot(aes(x = num_trials, y = interval_range, color = from_prior)) +
  # Use alpha to handle the high density of points where the curves overlap
  geom_point(alpha = 0.2) +
  labs(x = "Number of Trials (Targets)", 
       y = "Posterior 90% Interval Range", 
       title = "Sensitivity Analysis: Uncertainty Range by Prior Choice",
       color = "Prior Type") +
  theme_bw()
```
...
What do you think?  
The visualizations in this exam illustrate that an informative prior has its most significant impact when individual sample sizes are small. For players with few targets, the Empirical Bayes prior acts as a strong "anchor," shrinking noisy individual estimates toward the league average of approximately 0.67 and dramatically reducing the width of uncertainty intervals compared to the uninformed model. This "borrowing of strength" from the population compensates for the lack of group-specific data. But, as the sample size increases, this influence diminishes. For players with hundreds of targets, the likelihood dominates the prior, and the posterior estimates for both the uninformed and informative models converge on the same values. Ultimately, the informative prior provides a safety net for rare events while allowing the data to drive the conclusions for established veterans.
...
post_player_eval <- summary_post_df_all_empbayes %>% 
  mutate(
    # P(mu > 0.67) = 1 - pbeta(0.67, shape1, shape2)
...
post_player_eval %>%
  ...
  slice_head(n = 10) %>%
  select(player_id, num_trials, num_events, prob_grt_67)
```

The top 10 players consistently demonstrate both high efficiency and significant volume, as evidenced by their high observed catch rates and large sample sizes, with trial counts ranging from 122 up to 429. In this Bayesian framework, a high probability of exceeding the benchmark ( $P(\mu > 0.67)$ ) requires not only an elite posterior mean but also high statistical certainty. This is only achieved when the number of trials is large enough to narrow the uncertainty interval. Therefore, these players represent the league's most proven "high-volume" performers, where the sheer amount of data dominates the prior and confirms their elite status.
...
Add as many code chunks as you feel are necessary.  
```{r, solution_05c}
# Identify the 10 players with the lowest posterior probability of exceeding 0.67
...
# Identify the smallest number of targets in this group
min_trials_bottom_10 <- min(bottom_10_players$num_trials)
min_trials_bottom_10
```

The smallest number of targets (trial size) associated with these 10 players is 47. This group is primarily composed of players with high trial counts, such as player 545, who caught only 106 out of 207 targets (~0.51). This demonstrates that for the posterior probability of exceeding the benchmark to drop significantly toward zero (e.g., $4.26 \times 10^{-6}$), the model requires a substantial volume of data to confirm the underperformance. A player with a very small sample size and poor results would experience more shrinkage toward the league average of 0.67. This maintains a higher level of uncertainty and a higher $P(\mu > 0.67)$ than these proven veterans.
...
Add as many code chunks as you feel are necessary.  
```{r, solution_05d}
# Identify 10 players with smallest sample sizes and prob_grt_67 > 0.75
hidden_gems <- post_player_eval %>% 
  filter(prob_grt_67 > 0.75) %>% 
  slice_min(num_trials, n = 10) %>% 
  select(player_id, num_trials, num_events, post_avg, prob_grt_67)
...
What do you think?  
The focus on the 0.67 threshold is significant because this value represents the mean of the informative Empirical Bayes prior, which acts as the global statistical baseline for the entire league. As the hint suggests, this value is effectively equal to the completely pooled MLE, representing the single point estimate of catch probability if we assumed every player in the NFL possessed the exact same skill level.
  The interpretation of calculating $P(\mu > 0.67)$ is determining the statistical confidence that a specific player is a "better-than-average" performer. By using this benchmark, the team is asking: "What is the probability that this player's true underlying ability exceeds the performance of a generic, pooled NFL receiver?". This approach is superior to ranking by raw catch rates because it inherently accounts for sample size. A high probability requires not just an elite average, but enough trials to narrow the uncertainty interval sufficiently above the league-wide mean.
...
post_player_eval %>% 
  mutate(interval_range = post_q95 - post_q05) %>% 
  ggplot(aes(x = post_avg, y = interval_range)) +
  geom_point(aes(color = num_trials), alpha = 0.6) +
  scale_color_viridis_c() +
  labs(x = "Posterior Mean Catch Rate", 
       y = "Posterior 90% Interval Range (Uncertainty)", 
       color = "Number of Trials",
       title = "Risk-Reward Profile: Uncertainty vs. Posterior Mean") +
  theme_bw()
```
...
What do you think?  
Based on the Risk-Reward Profile visualized in Problem 5f, I would recommend that the team take a risk on players located in the top-right quadrant of the plot. These players exhibit high posterior mean catch rates often exceeding the 0.67 league benchmark, but also possess the highest 90% uncertainty intervals (ranging from approximately 0.25 to 0.32) due to their small sample sizes. While their limited targets make them statistically uncertain, their high on-field efficiency suggests significant upside that hasn't yet been fully proven over a large number of trials. By signing these players to incentive-heavy contracts with lower guaranteed money, the team can effectively capitalize on their high potential reward. This strategy statistically manages the risk that their performance might eventually regress toward the 0.67 league average as their trial count increases.