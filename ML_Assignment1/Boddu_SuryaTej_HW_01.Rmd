---
title: "INFSCI 2595 Fall 2025 Homework: 01"
subtitle: "Assigned August 28, 2025; Due: September 3, 2025"
author: "Surya Tej Boddu"
date: "Submission time: September 3, 2025 at 11:59PM EST"
...
```{r, solution_01a}
ggplot(iris, aes(x = Sepal.Length)) + geom_histogram(bins = 6)
```
...
```{r, solution_01b}
ggplot(iris, aes(x = Sepal.Length)) +  geom_histogram(bins = 6, color = "black")
```
...
```{r, solution_01c}
ggplot(iris, aes(x = Sepal.Length)) +  geom_histogram(bins = 6, color = "navyblue", fill = "gold")
```
...
```{r, solution_01d}
ggplot(iris, aes(x = Sepal.Length)) +  geom_histogram(bins = 6, color = "navyblue", fill = "gold", size = 1.55)
```
...
The boxplot shows a counterintuitive pattern where diamonds with better color grades (D, E, F) tend to have lower median prices compared to diamonds with worse color grades (I, J). Specifically, the median price increases as color quality decreases from D to J. This is surprising because better color should typically command higher prices. However, this relationship is likely confounded by other important variables such as carat weight, cut quality, and clarity. It's possible that smaller diamonds tend to have better color grades, while larger (and thus more expensive) diamonds may have worse color grades, which would explain this inverse relationship when other factors are not controlled for.
...
```{r, solution_02c}
diamonds %>% ggplot(aes(x = color, y = price)) +  geom_boxplot() +  facet_wrap(~cut)
```
...
```{r, solution_02d}
diamonds %>% ggplot(aes(x = color, y = price)) +  geom_boxplot() +  facet_grid(clarity ~ cut)
```
...
```{r, solution_02e}
diamonds %>% ggplot(aes(x = color, y = price)) +  geom_boxplot(aes(fill = carat > median(carat))) +  facet_grid(clarity ~ cut) +  theme(legend.position = "top")
```
...
```{r, solution_02f}
diamonds %>% filter(cut == "Ideal", clarity == "IF") %>%  ggplot(aes(x = color, y = price)) +  geom_boxplot(aes(fill = carat > median(diamonds$carat))) +  facet_grid(clarity ~ cut) +  theme(legend.position = "top")
```
...
When comparing these two plots, the first one (2b) showed an unexpected result where diamonds with better color grades like D appeared to cost less than those with worse grades like J. However, in the filtered plot (2f) focusing on Ideal cut and IF clarity, we can see that color D diamonds actually tend to be more expensive overall, especially for larger stones, which makes more sense intuitively. My takeaway is that color D is indeed more valuable than color J, but in the initial plot this relationship was hidden because multiple factors were influencing the price at once. Once we narrow down to specific cut and clarity levels, the effect of color on price becomes much clearer.
...
```{r, solution_03b}
mpg %>% ggplot(aes(x = displ, y = hwy)) +  geom_point() +  geom_smooth()
```
...
```{r, solution_03c}
mpg %>% ggplot(aes(x = displ, y = hwy)) +  geom_point() +  geom_smooth(method = lm)
```
...
```{r, solution_03d}
mpg %>% ggplot(aes(x = displ, y = hwy)) +  geom_point() +  geom_smooth(method = lm, formula = y ~ x)
```
...
```{r, solution_03e}
mpg %>% ggplot(aes(x = displ, y = hwy)) +  geom_point() +  geom_smooth(aes(color = drv), method = lm, formula = y ~ x)
```
...
```{r, solution_03f}
mpg %>% ggplot(aes(x = displ, y = hwy)) +  geom_point() +  geom_smooth(aes(group = drv), method = lm, formula = y ~ x)
```
