---
title: "INFSCI 2595 Fall 2025 Homework: 02"
subtitle: "Assigned September 4, 2025; Due: September 10, 2025"
author: "Surya Tej Boddu"
date: "Submission time: September 10, 2025 at 11:59PM EST"
...
```{r, solution_01a}
ggplot(df_low_high, aes(x = x, y = y)) +  geom_point(size = 3)
```

**Is the relationship linear or non-linear?**  
The relationship appears non-linear. There is a visible curvature (likely a parabola).
...
```{r, solution_01c}
cat("Degree 1 RMSE:", modelr::rmse(fit_lm_1, df_low_high), "\n")
cat("Degree 2 RMSE:", modelr::rmse(fit_lm_2, df_low_high), "\n")
cat("Degree 3 RMSE:", modelr::rmse(fit_lm_3, df_low_high), "\n")
cat("Degree 4 RMSE:", modelr::rmse(fit_lm_4, df_low_high), "\n")
cat("Degree 5 RMSE:", modelr::rmse(fit_lm_5, df_low_high), "\n")
cat("Degree 6 RMSE:", modelr::rmse(fit_lm_6, df_low_high), "\n")
cat("Degree 7 RMSE:", modelr::rmse(fit_lm_7, df_low_high), "\n")
cat("Degree 8 RMSE:", modelr::rmse(fit_lm_8, df_low_high), "\n")
cat("Degree 9 RMSE:", modelr::rmse(fit_lm_9, df_low_high), "\n")
```
...
```{r, solution_01d}
cat("Degree 1 R-squared:", modelr::rsquare(fit_lm_1, df_low_high), "\n")
cat("Degree 2 R-squared:", modelr::rsquare(fit_lm_2, df_low_high), "\n")
cat("Degree 3 R-squared:", modelr::rsquare(fit_lm_3, df_low_high), "\n")
cat("Degree 4 R-squared:", modelr::rsquare(fit_lm_4, df_low_high), "\n")
cat("Degree 5 R-squared:", modelr::rsquare(fit_lm_5, df_low_high), "\n")
cat("Degree 6 R-squared:", modelr::rsquare(fit_lm_6, df_low_high), "\n")
cat("Degree 7 R-squared:", modelr::rsquare(fit_lm_7, df_low_high), "\n")
cat("Degree 8 R-squared:", modelr::rsquare(fit_lm_8, df_low_high), "\n")
cat("Degree 9 R-squared:", modelr::rsquare(fit_lm_9, df_low_high), "\n")
```
...
What do you think?  
Based on the training set performance, the 9th-degree polynomial model is the best-performing model. This model has the lowest training RMSE and the highest training R-squared among all models. Both performance metrics therefore agree on the best model when evaluated using the training data alone.
But, this result is expected because more complex models can closely fit the training data, especially in the presence of noise. This selection does not necessarily mean that the 9th-degree model will generalize well to new data, as higher-degree models are more prone to overfitting.
...
What do you think? 
1) The coefficient estimates are not identical across models. While lower-order terms (such as x and x^2) remain relatively similar in magnitude between the quadratic and 6th-degree models, they become less stable in the 9th-degree model as additional polynomial terms are introduced.
2) The level of uncertainty is not consistent across models. Uncertainty increases with model complexity i.e., lower-degree coefficients tend to have narrower confidence intervals, while medium- and high-degree coefficients exhibit much wider intervals, often spanning zero. This indicates that higher-degree terms are weakly supported and difficult to estimate reliably in the presence of noise.
...
viz_trend_1 <- predict(fit_lm_1, newdata = input_viz)
```

```{r, solution_02b_b, eval=TRUE}
### quadratic relationship (degree 2) predictions
viz_trend_2 <- predict(fit_lm_2, newdata = input_viz)
```

```{r, solution_02b_c, eval=TRUE}
### cubic relationship (degree 3) predictions
viz_trend_3 <- predict(fit_lm_3, newdata = input_viz)
```

```{r, solution_02b_d, eval=TRUE}
### degree 4
viz_trend_4 <- predict(fit_lm_4, newdata = input_viz)
```

```{r, solution_02b_e, eval=TRUE}
### degree 5
viz_trend_5 <- predict(fit_lm_5, newdata = input_viz)
```

```{r, solution_02b_f, eval=TRUE}
### degree 6
viz_trend_6 <- predict(fit_lm_6, newdata = input_viz)
```

```{r, solution_02b_g, eval=TRUE}
### degree 7
viz_trend_7 <- predict(fit_lm_7, newdata = input_viz)
```

```{r, solution_02b_h, eval=TRUE}
### degree 8
viz_trend_8 <- predict(fit_lm_8, newdata = input_viz)
```

```{r, solution_02b_i, eval=TRUE}
### degree 9
viz_trend_9 <- predict(fit_lm_9, newdata = input_viz)
```
...
```{r, solution_02d}
ggplot(df_viz_1, aes(x = x)) +  geom_line(aes(y = pred_trend))
```
...
```{r, solution_02e}
ggplot(df_viz_1, aes(x = x)) +  geom_point(data = df_low_high, aes(y = y), shape = 0, color = "red") +  geom_line(aes(y = pred_trend), size = 1)
```
...
```{r, solution_02h}
df_viz_all %>% ggplot(aes(x = x)) + geom_line(aes(y = pred_trend)) + facet_wrap(~ poly_degree)
```
...
```{r, solution_02i}
df_viz_all %>% ggplot(aes(x = x)) + geom_line(aes(y = pred_trend)) + facet_wrap(~ poly_degree, scales = "free_y")
```
...
```{r, solution_02j}
df_viz_all %>% ggplot(aes(x = x)) + geom_point(data = df_low_high, aes(y = y), shape = 0, color = "red") + geom_line(aes(y = pred_trend)) + facet_wrap(~ poly_degree, scales = "free_y")
```
...
What do you think?  
The best-performing model is fit_lm_9. Its predicted trend is relatively stable at lower values of x but bends sharply near the boundary to accommodate extreme observations.
The trend is consistent with the training data. But, the excessive flexibility suggests that the model may be overfitting.
...
```{r, solution_03b}
df_split <- bind_rows(
  df_train %>% mutate(split = "Training"),
  df_holdout %>% mutate(split = "Hold-out")
)

ggplot(df_split, aes(x = x, y = y, color = split, shape = split)) +
  geom_point(size = 3) +
  theme_bw()
```
...
```{r, solution_03d}
cat("Degree 1 Hold-out RMSE:", modelr::rmse(fit_train_1, df_holdout), "\n")
cat("Degree 2 Hold-out RMSE:", modelr::rmse(fit_train_2, df_holdout), "\n")
cat("Degree 3 Hold-out RMSE:", modelr::rmse(fit_train_3, df_holdout), "\n")
cat("Degree 4 Hold-out RMSE:", modelr::rmse(fit_train_4, df_holdout), "\n")
cat("Degree 5 Hold-out RMSE:", modelr::rmse(fit_train_5, df_holdout), "\n")
cat("Degree 6 Hold-out RMSE:", modelr::rmse(fit_train_6, df_holdout), "\n")
cat("Degree 7 Hold-out RMSE:", modelr::rmse(fit_train_7, df_holdout), "\n")
cat("Degree 8 Hold-out RMSE:", modelr::rmse(fit_train_8, df_holdout), "\n")
cat("Degree 9 Hold-out RMSE:", modelr::rmse(fit_train_9, df_holdout), "\n")
```
...
What do you think? 
According to the hold-out RMSE, the quadratic model (degree 2) is the best-performing model, as it has the lowest hold-out RMSE (7.88).
  This model is not the same as the model identified as best using training-set performance, which favored a much higher-degree polynomial. The training-best model performs very poorly on the hold-out set, ranking among the worst models with extremely large RMSE values.
  This discrepancy indicates that the higher-degree models are overfitting the training data, while the quadratic model achieves a better balance between bias and variance and therefore generalizes better to new data.
...
What do you think?  
This comparison demonstrates that the 9th-degree model is unreliable. A good model should yield similar coefficient estimates even if the training data changes slightly. The high sensitivity of coefficients observed here is a symptom of overfitting to a specific sample rather than learning the true underlying population trend.
...
What do you think? 
The coefficients from the two fits are very similar and stable. For the intercept, x, and x^2, the point estimates are nearly identical and the confidence intervals largely overlap between fit_lm_2 (full data) and fit_train_2 (80% training split). The uncertainty levels are also comparable and narrow. This stability indicates that the quadratic model has low variance and is robust to small changes in the training data. This supports its robust performance on the hold-out set compared to higher-degree models.
...
5 times.
In 5-fold cross-validation, the dataset is partitioned into 5 equal-sized folds. The training process repeats 5 times. In each iteration, a different fold is taken as the test set while the model is trained on the remaining 4 folds.
...
I chose RMSE because it is the standard metric for regression. RMSE quantifies prediction error in the same units as the response variable, directly measuring how much our predictions deviate from the actual data.
...
The best model is the cubic model (degree 3, fit_03). It has the lowest mean cross-validated RMSE (~5.07) among all models.The dotplot also shows it performing best with relatively low variability.
...
What do you think?  
1) For coefficients that appear in at least two models (Intercept, x, x^2), the estimates are similar and their confidence intervals largely overlap, indicating stability across models.
2) The level of uncertainty is consistent across the three models, with relatively narrow intervals for lower-order terms; adding x^3 does not introduce excessive uncertainty.
...
```{r, solution_05a}
ggplot(data = df_low_low) + geom_point(aes(x = x, y = y), size = 3)
```

How does it compare?  
Compared to the high-noise data in Problem 1a, this plot shows much less scatter and a clear non-linear (U-shaped) relationship between x and y. The underlying trend is more visible, which indicates lower noise. This makes the true data-generating process easier to identify.
...
Which model is the best?  
The best model is the 6th-degree polynomial (fit_06). It has the lowest mean cross-validated RMSE (~1.05) among all models in the summary table. The dotplot also places fit_06 at the lowest RMSE with relatively small uncertainty compared to higher-degree models.
...
What do you think?  
No, the resampling scheme did not identify the same best model for the two noise cases. In the high-noise case, simpler models (low-degree polynomials) performed best, while higher-degree models performed poorly due to overfitting. In the low-noise case, a more complex model (degree 6) was identified as the best, because the reduced noise allowed the model to learn more detailed pattern in the data.
The models that were poor in the high-noise case (very high-degree polynomials) did not remain poor in the low-noise case - their performance improved substantially.
...
```{r, solution_06a}
ggplot(data = df_high_high) + geom_point(aes(x = x, y = y), size = 3)
```

How does it compare?  
Compared to Problem 1a (low sample size, high noise), this plot still shows high noise, but the much larger sample size makes the underlying non-linear pattern clearer. The increased number of observations helps reveal the true trend despite the noise, making complex behavior easier to detect than in the small-sample case.
...
Which model is the best?  
The best model is the 5th-degree polynomial (fit_05). It has the lowest mean cross-validated RMSE (~4.33) among all models. The dotplot also places fit_05 at the lowest RMSE with relatively low uncertainty.
...
What do you think?  
1) No. With low sample size, the 3rd-degree (cubic) model was optimal, whereas with high sample size, the resampling scheme identified the 5th-degree model as the best choice.
2) It depends on why they were poor.Underfitting models (degree 1) remained poor because increased sample size cannot fix a model that is structurally too simple to capture the U-shaped relationship.Overfitting models (degrees 7–9) improved substantially as sample size increased, but they remained sub-optimal due to higher variance and greater uncertainty compared to the 5th-degree model.
...
What do you think?  
Model complexity that can be learned depends on both noise and sample size. High noise favors simpler models, while larger samples reduce variance and allow more complex models. Complex pattern is easiest to identify with low noise and large sample size.
