---
title: "INFSCI 2595 Fall 2025 Homework: 03"
subtitle: "Assigned September 11, 2025; Due: September 17, 2025"
author: "Surya Tej Boddu"
date: "Submission time: September 17, 2025 at 11:59PM EST"
...
Yes! the data set is approximately balanced, because the event rate is close to 50%. The numbers of event and non_event observations are nearly equal.
...
```{r, solution_01d}
model_pred_df %>% ggplot(aes(x=x, y=y)) + geom_point(size = 3.5, alpha = 0.5)
```
...
```{r, solution_03f}
model_pred_df %>% ggplot(aes(x=x)) + geom_line(aes(y=pred_prob), color="red") + geom_point(aes(y=y), size=3.5, alpha=0.5)
```
...
What do you think?  
The observed binary response does follow the model predicted probability.
...
```{r, solution_02b}
model_class_0.5 %>% summarize(accuracy = mean(pred_class == obs_class))
```
...
your response here.  
Based on the counts, there are 6 observations associated with False-Positives and 53 observations associated with True-Negatives.
...
```{r, solution_02d}
metrics_0.5 <- model_class_0.5 %>% 
  summarize(
    TP = sum(pred_class == "event" & obs_class == "event"),
    FN = sum(pred_class == "non_event" & obs_class == "event"),
    FP = sum(pred_class == "event" & obs_class == "non_event"),
    TN = sum(pred_class == "non_event" & obs_class == "non_event"),
...
```{r, solution_03a}
model_class_0.7 %>% dplyr::summarize(accuracy=mean(pred_class==obs_class))
```
...
```{r, solution_03b}
metrics_0.7 <- model_class_0.7 %>% 
  dplyr::summarize(
    Sensitivity = mean(pred_class == "event" & obs_class == "event") / mean(obs_class == "event"),
    Specificity = mean(pred_class == "non_event" & obs_class == "non_event") / mean(obs_class == "non_event")
  )
...
```{r, solution_03c}
model_class_0.3 %>% dplyr::summarize(accuracy=mean(pred_class==obs_class))
```
...
roc_results <- tibble::tibble(
  ...
  fpr = c(1 - metrics_0.3$Specificity, 1 - metrics_0.5$Specificity, 1 - metrics_0.7$Specificity),
  sens = c(metrics_0.3$Sensitivity, metrics_0.5$Sensitivity, metrics_0.7$Sensitivity)
)

roc_results %>% 
  ggplot(mapping = aes(x = fpr, y = sens)) +
  geom_point(size = 4) +
  geom_abline(slope = 1, intercept = 0, linetype = "dashed", color = "grey") +
  coord_equal(xlim = c(0, 1), ylim = c(0, 1)) +
  labs(x = "False Positive Rate (1 - Specificity)",
       y = "Sensitivity (True Positive Rate)",
       title = "3-Point ROC Curve") +
  theme_bw()
```
...
Yes, the binary outcome is balanced. Since 0.483871 is very close to 0.5 (50%), the classes are represented almost equally in the dataset.
...
```
- Top 2 Models (mod_3, mod_2) exhibit the lowest error rates. They minimize both False-Positives and False-Negatives.
- Worst 2 Models (mod_8, mod_4) are more likely to trigger False-Positives and False-Negatives due to less predictive features.
...
Which model is the best?  
Model 3 (mod_3_roc) is the best model.
...
- Model 2 (mod_2_roc) has the highest True-Positive Rate (Sensitivity).
- Model 3 (mod_3_roc) has the lowest False-Positive Rate (1 - Specificity).
...
```{r, solution_05e}
mod_1_roc$pred %>% yardstick::roc_curve(truth = obs, event)
```


What do you think?  
At a very low threshold, the model predicts event for almost all observation. This eliminates False Negatives (FN = 0) because every actual event is captured by the liberal threshold. Since Sensitivity = TP/(TP + FN), having zero False Negatives results in a Sensitivity of 1.0. But, this comes at the cost of Specificity, which decreases to 0 because all non-events are also classified as events.
...
```{r, solution_05f}
mod_1_roc$pred %>% 
  yardstick::roc_curve(truth = obs, event) %>% ggplot2::autoplot()
```
...
What do you think?  
- Model 3 is the best model. Its curve is the furthest toward the top-left corner, indicating the highest overall performance.
- The result is consistent with the previous ROC AUC summary metrics where Model 3 (and Model 2) showed the highest mean Area Under the Curve.
- Model 8 is the closest to a "completely ineffective model" as its curve sits closest to the diagonal dashed line. A model on this line is akin to random guessing as it can't distinguish between an "event" and a "non_event".
...
What do you think?  
The relationship is complementary. For every row, the two values must sum to 1.
...
```{r, solution_06e}
test_df_1_b %>% count(pred_bin) %>% ggplot(aes(x=pred_bin, y=n)) + geom_col()
```
...
What do you think?  
The bins at the lowest predicted probabilities ([0, 0.1]) consist almost entirely of non-events, while the bins at the highest predicted probabilities ((0.9, 1]) consist almost entirely of events. This confirms the model is effectively separating the two classes. Whether a bin contains only one class depends on the specific test set results, but typically the extreme ends show high purity, whereas the middle bins (near 0.5) show a mixed distribution of both outcomes.

```{r, solution_06f}
test_df_1_b %>% 
  ggplot(aes(x = pred_bin, fill = y)) +
  geom_bar() +
...
What do you think?  
No, the empirical proportions for Model 1 are not consistent with its predicted probabilities. For the first three bins where the model predicts a low probability of an event (0 to 0.3), the actual observed event rate is significantly higher. Because the red "event" portion of these bars is much larger than the values on the x-axis, the model is under-predicting the actual likelihood of the event in those ranges.
...
my_calcurve_1 <- test_df_1_b %>%
  ...
  mutate(
    mid_bin = case_when(
      pred_bin == "[0,0.1]" ~ 0.05,
      pred_bin == "(0.1,0.2]" ~ 0.15,
      pred_bin == "(0.2,0.3]" ~ 0.25,
      pred_bin == "(0.3,0.4]" ~ 0.35,
      pred_bin == "(0.4,0.5]" ~ 0.45,
      pred_bin == "(0.5,0.6]" ~ 0.55,
      pred_bin == "(0.6,0.7]" ~ 0.65,
      pred_bin == "(0.7,0.8]" ~ 0.75,
      pred_bin == "(0.8,0.9]" ~ 0.85,
      pred_bin == "(0.9,1]" ~ 0.95
    )
  )
```
...
What do you think?  
Yes, it is consistent with the stacked filled bar chart.
...
my_calcurve_3 <- test_df_3_b %>%
  group_by(pred_bin) %>%
  summarize(prop_event = mean(y == "event")) %>%
  mutate(mid_bin = case_when(
    pred_bin == "[0,0.1]" ~ 0.05,
    pred_bin == "(0.1,0.2]" ~ 0.15,
    pred_bin == "(0.2,0.3]" ~ 0.25,
    pred_bin == "(0.3,0.4]" ~ 0.35,
...
What do you think?  
- Model 3 is the most well-calibrated because its points track the dashed diagonal line more closely and across a wider range of probabilities.
- Yes, the differences are obvious. Model 1 shows a clear systematic under-prediction bias at the low end, while Model 8 lacks the range to predict high/low probabilities and fails as the predicted probability increases.
- The most obvious aspects are reliability in Model 3 (actual outcomes match predictions), systematic bias in Model 1 (actual event rates are higher than predicted), and poor discrimination in Model 8 (it clusters predictions in the middle and fails at the extremes).
...
Yes, the curve is consistent with the manually created curve from the previous problems.
