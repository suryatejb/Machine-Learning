---
title: "INFSCI 2595 Fall 2025 Homework: 05"
subtitle: "Assigned September 25, 2025; Due: October 1, 2025"
author: "Surya Tej Boddu"
date: "Submission time: October 1, 2025 at 11:59PM EST"
...
$$
[\mu_0-2\tau_0, \mu_0+2\tau_0] = [115, 135]
$$
$$
\mu_0 - 2\tau_0 = 115,  \space  \mu_0 + 2\tau_0 = 135
$$
$$
2\mu_0 = 115 + 135
$$
...
$$
$$
\tau_0 = \frac{(135 - 125)}{2} = 5
$$
$$
\mu \sim \mathcal {N}(125, 5^2)
$$
...
The formula for finding a quantile in a normal distribution is:
$$
Q_{p} = \mu_0 + z_{p} \cdot \tau_0
$$
25th Quantile:
$$
Q_{25} = \mu_0 + z_{0.25} \cdot \tau_0 = 125 + (-0.674) \cdot 5 = = 125−3.37245 \approx 121.63
$$
75th quantile:
$$
Q_{75} = \mu_0 + z_{0.75} \cdot \tau_0 = 125 + (0.674) \cdot 5 = 125+3.37245 \approx 128.37
$$
Probability mass between them:
$$
P(Q_{0.25} \leq \mu \leq Q_{0.75}) = 0.75 - 0.25 = 0.50
$$
The interval between the 25th and 75th quantiles contains exactly 50% of the probability mass.
...
$$
z = \frac{117 - 125}{5} = \frac{-8}{5} = -1.6
$$

$$
P(\mu \leq 117) = P(Z \leq -1.6) \approx 0.0548 \approx 5.5\%
$$
...
$$
N = \sigma^2 \left(\frac{1}{\tau_N^2} - \frac{1}{\tau_0^2}\right)
$$
**Case 1: Target $\tau_N = 2$**

$$
N = 5^2 \left(\frac{1}{2^2} - \frac{1}{5^2}\right) = 25 \left(0.21\right) = 5.25
$$
Since we cannot have a fraction of an observation, we round up to 6 observations.

**Case 2: Target $\tau_N = 0.5$**

$$
N = 5^2 \left(\frac{1}{0.5^2} - \frac{1}{5^2}\right) = 25 \left(3.96\right) = 99
$$
Exactly 99 observations are required.
...
Posterior variance formula (normal likelihood, known $\sigma$):
$$
\frac{1}{\tau_N^2} = \frac{1}{\tau_0^2} + \frac{N}{\sigma^2}
$$
With an infinitely diffuse prior $\tau_0 \rightarrow \infty$:
$$
\frac{1}{\tau_0^2} \rightarrow 0
$$
So it simplifies to:
$$
N = \frac{\sigma^2}{\tau_N^2}
$$
We know that $\sigma = 15$ and $\tau_N = 3$:
$$
N = \frac{15^2}{3^2} = \frac{225}{9} = 25
$$
25 observations are required to achieve a posterior standard deviation of 3 under these conditions.
...
$$
p(y) = \frac{1}{\sqrt{2\pi \text{Var}}} \exp\left(-\frac{(y - \text{mean})^2}{2\text{Var}}\right)
$$
Substituting the likelihood and prior:
$$
p(\mu \mid \mathbf{x}, \sigma) \propto \left[\prod_{n=1}^{N} \frac{1}{\sigma\sqrt{2\pi}} 
\exp\left(-\frac{(x_n - \mu)^2}{2\sigma^2}\right)\right] \times 
\frac{1}{\tau_0\sqrt{2\pi}} \exp\left(-\frac{(\mu - \mu_0)^2}{2\tau_0^2}\right)
$$
Take log on both sides:
$$
\log p(\mu \mid \mathbf{x}, \sigma) \propto \sum_{n=1}^{N} \log\left(\frac{1}{\sigma\sqrt{2\pi}} 
\exp\left(-\frac{(x_n - \mu)^2}{2\sigma^2}\right)\right) + 
\log\left(\frac{1}{\tau_0\sqrt{2\pi}} \exp\left(-\frac{(\mu - \mu_0)^2}{2\tau_0^2}\right)\right)
$$
Terms like $\log(\sigma\sqrt{2\pi})$ and $\log(\tau_0\sqrt{2\pi})$ are constants with respect to $\mu$. Dropping them:
$$
\log p(\mu \mid \mathbf{x}, \sigma) \propto \sum_{n=1}^{N} 
\left(-\frac{(x_n - \mu)^2}{2\sigma^2}\right) - \frac{(\mu - \mu_0)^2}{2\tau_0^2}
$$
$$
\log p(\mu \mid \mathbf{x}, \sigma) \propto -\frac{1}{2\sigma^2}\sum_{n=1}^{N}(x_n - \mu)^2 
...
The derivative of the first term (likelihood part) and second term (prior part):
$$
\frac{d}{d\mu}\left[-\frac{1}{2\sigma^2}\sum_{n=1}^{N}(x_n - \mu)^2\right] = 
-\frac{1}{2\sigma^2}\sum_{n=1}^{N} 2(x_n - \mu)(-1) = 
\frac{1}{\sigma^2}\sum_{n=1}^{N}(x_n - \mu)
$$
$$
\frac{d}{d\mu}\left[-\frac{1}{2\tau_0^2}(\mu - \mu_0)^2\right] = 
-\frac{1}{2\tau_0^2} \cdot 2(\mu - \mu_0)(1) = 
-\frac{1}{\tau_0^2}(\mu - \mu_0)
$$
Combine the terms:
$$
\frac{d}{d\mu}\log p(\mu \mid \mathbf{x}, \sigma) = 
\frac{1}{\sigma^2}\sum_{n=1}^{N}(x_n - \mu) - \frac{1}{\tau_0^2}(\mu - \mu_0)
$$
Expanding the summation in the first term:

$$
\sum_{n=1}^{N}(x_n - \mu) = \sum_{n=1}^{N} x_n - \sum_{n=1}^{N}\mu = 
N\bar{x} - N\mu = N(\bar{x} - \mu)
$$
Final expression:
$$
\frac{d}{d\mu}\log p(\mu \mid \mathbf{x}, \sigma) = 
...
$$
0 = \frac{N}{\sigma^2}(\bar{x} - \mu_{MAP}) - \frac{1}{\tau_0^2}(\mu_{MAP} - \mu_0)
$$
$$
0 = N\tau_0^2(\bar{x} - \mu_{MAP}) - \sigma^2(\mu_{MAP} - \mu_0)
$$
$$
\mu_{MAP}(N\tau_0^2 + \sigma^2) = N\tau_0^2\bar{x} + \sigma^2\mu_0
$$
$$
...
My $\mu_{MAP}$ expression is the same as the posterior mean $\mu_N$ from lecture (the mean of the conjugate Normal posterior in the normal-normal model). Because in this model the posterior $p(\mu \mid \mathbf{x}, \sigma)$ is itself Gaussian. For a Gaussian distribution, the mode = mean (and also = median). So the MAP (posterior mode) must equal the posterior mean.
...
Log-Likelihood term:
$$
\sum_{n=1}^{N} \log(\text{normal}(x_n \mid \mu, \sigma)) \propto 
-N\log(\sigma) - \frac{1}{2\sigma^2}\sum_{n=1}^{N}(x_n - \mu)^2
$$
Log-Prior term:
$$
\log \text{Exp}(\sigma \mid \lambda) \propto -\lambda\sigma
$$
Total Un-normalized Log-Posterior:
$$
\log p(\sigma \mid \mathbf{x}, \mu) \propto 
-\frac{1}{2\sigma^2}\sum_{n=1}^{N}(x_n - \mu)^2 - N\log(\sigma) - \lambda\sigma
$$
...
$$
\sum_{n=1}^{N}(x_n - \mu)^2 = N \cdot v
$$
Substitute $N \cdot v$ into the summation term from 3a result:
$$
\log p(\sigma \mid \mathbf{x}, \mu) \propto -\frac{Nv}{2\sigma^2} - N\log(\sigma) - \lambda\sigma
$$
...
{
  log_post <- - (N * v) / (2 * sigma^2) - N * log(sigma) - lambda * sigma
...
tibble::tibble(
  ...
  mutate(log_post = log_post_sigma_unnorm(sigma, N = 21, v = 4.2, lambda = 0.75)) %>%
  ggplot(aes(x = sigma, y = log_post)) + 
  geom_line()
```

What do you think?  
- The log-posterior has one mode (a single clear peak).
- It does not look like a parabola. While it has a peak, the shape is asymmetric; it rises very steeply from the left (near zero) and then has a much shallower, more gradual decline as $\sigma$ increases.
...
$$
$$
\frac{d\sigma}{d\varphi} = \frac{d}{d\varphi}(e^{\varphi}) = e^{\varphi}
$$
$$
\log\left(\left|\frac{d\sigma}{d\varphi}\right|\right) = \log(e^{\varphi}) = \varphi
$$
...
log_post_varphi_unnorm <- function(varphi, N, v, lambda)
{
  ...
  # account for the derivative adjustment (add log|dsigma/dvarphi|)
...
tibble::tibble(
  ...
  mutate(log_post = log_post_varphi_unnorm(varphi, N = 21, v = 4.2, lambda = 0.75)) %>% 
...
What do you think?  
Yes. Compared to the graph in 3d, the log-posterior for $\varphi$ appears much more symmetric and has a shape that closely resembles a parabola. This transformation effectively stretches the lower values and compresses the higher ones, making the Gaussian approximation much more accurate in this space.
...
print(opt_sine_01)
```
Which control argument tells optim() to maximize instead of minimize?
The argument is control = list(fnscale = -1). Setting fnscale to a negative value flips the objective function, effectively turning a minimization problem into a maximization problem.

What should gr be set to?
Since we are not passing in an analytical gradient function, gr should be set to NULL. The optim() function will then use numerical methods to estimate the gradient.
...
opt_result <- optim(par = 1.75,
                    function(x) my_func(x, my_constants),
                    gr = NULL,
                    method = "Brent",
                    lower = -0.5, upper = 2,
                    hessian = TRUE,
...
varphi_opt_a <- optim(-0.5,
                      function(varphi) log_post_varphi_unnorm(varphi, N, v, lambda = 0.75),
                      gr = NULL,
                      method = "BFGS",
                      control = list(fnscale=-1))
print(varphi_opt_a)
...
```
Is the posterior mode from the initial guess of 1.35 different from the initial guess of -0.5?
No, the results should be virtually identical. We see that varphi_opt_b matches varphi_opt_a to several decimal places.
...
tibble::tibble(
  ...
  ggplot(aes(x = varphi, y = log_post)) + 
  geom_line() + 
  # Plotting the result from the second optimization run
  geom_vline(xintercept = varphi_opt_b$par, color = "red")
```

I performed the optimization twice using different starting values (-0.5 and 1.35) to ensure the algorithm wasn't getting stuck in a local optimum. Both runs converged to the same MAP estimate, confirming the posterior is unimodal.